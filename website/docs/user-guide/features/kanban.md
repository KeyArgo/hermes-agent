...
The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `hermes profile create --description "..."`, `hermes profile describe <name> --text "..."`, `hermes profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee`; when that is unset the child inherits the decomposed card's own assignee (the fleet's routing decision for that work), and only a card with no real profile assignee falls back to the active default profile.
...
| `default_assignee` | `""` | Where a child task lands when the LLM picks an unknown profile. Empty = inherit the decomposed card's assignee, then fall back to active default. |
